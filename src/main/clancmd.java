		...
		if (cmd.getName().equalsIgnoreCase("clan")) {
			if (!(sender instanceof Player)) {
				...
				if(args.length == 0) {
					...
					p.sendMessage("");			
					p.sendMessage("§8§l»§r §fPlugin by: §9@Te4hnoPlus and ReMuru");	
					p.sendMessage("");	
				} else {
					if(args[0].equalsIgnoreCase("help")) { p.performCommand("clan");	
									...
									if (!(plugin.getConfig().contains("clan." + clan))) {
										...
										if (args[1].length() <= sfile.getInt("clan_max_lenght") ) {													
											if (plugin.getEconomy().getBalance(p.getName()) >= sfile.getInt("cost")) {
												...
												Bukkit.broadcastMessage(prefix + lfile.getString("create.created").replaceAll("&", "Â§").replaceAll("%player%", p.getName()).replaceAll("%clan%", clan));
												plugin.saveConfig();
												plugin.reloadConfig();	
												clan_members.clear();										
						...
						} else {p.sendMessage(prefix + lfile.getString("global.perms").replaceAll("&", "Â§"));}	
